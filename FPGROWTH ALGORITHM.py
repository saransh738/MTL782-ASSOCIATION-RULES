...
def construct_FP(S_min,DataBase):
   ...
   # now we form frequent 1 itemset i.e. recurrent_set         
    ...
    for SET,freq in DataBase.items():
        recurrent_items = {}
        for element in SET:
            if element in recurrent_set: recurrent_items[element] = Reference_Table[element][0]
        if len(recurrent_items) != 0:
            ...
            while(i<n):
                ...
                if SET[i] not in tree.descendent:
                    ...
                    if Reference_Table[SET[i]][1] == None: Reference_Table[SET[i]][1] = tree.descendent[SET[i]]
                    else:
...
def FP_Mine(tree, Reference_Table, S_min, pathprefix, recurrent_set,frequency_set):
    ...
    for L in List:
        ...
        if c_header != None: FP_Mine(c_tree, c_header, S_min, new_recurrentset, recurrent_set,frequency_set)
...
def main(input_file,S_min):
    ...
    FP_Mine(tree, Reference_Table, S_min, set([]), recurrent_set,frequency_set)  #mining
